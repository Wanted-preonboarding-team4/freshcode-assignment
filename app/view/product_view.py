...
from app.service.product_service import is_menu_name_exist,is_menu_exist
...
def create_product(menu_info: MenuRegister, session: Session = Depends(db.session)):
    if not menu_info.category_id or not menu_info.menu_name or not menu_info.description or not menu_info.item:
